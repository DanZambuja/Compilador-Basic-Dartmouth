...
*/
void show_bmp(u8 *p, u32 start_row, u32 start_col, int pixel_color)
{
    u32 h, w, pixel, rsize, i, j;
    u32 *q =(u32 *)(p+14); //skip over 14-byte file header
    w = 800; // how many cols in pixel
    h = 60; // how may rows in pixel
    ...
    for(i = start_row; i<start_row + h; i++)
    {
        ...
        {
            fb[i*gDisplayContext.screen_width + j]=pixel_color; // in the frame buffer, a pixel occupies 32-bit.
...
void show_bmp_black(u8 *p, u32 start_row, u32 start_col)
{
    u32 h, w, pixel, rsize, i, j;
    u8 r, g, b;
    u8* pp;
    ...
    for (i = start_row; i < start_row + h; i++) {
        for (j = start_col; j < start_col + w; j++) {
            fb[i*gDisplayContext.screen_width + j]=0; // in the frame buffer, a pixel occupies 32-bit.
        }
        p -=rsize; // to preceding row
    }
}

void show_bmp_white(u8 *p, u32 start_row, u32 start_col)
{
    u32 h, w, pixel, rsize, i, j;
    u8 r, g, b;
    u8* pp;
    ...
    for (i = start_row; i < start_row + h; i++) {
        for (j = start_col; j < start_col + w; j++) {
            fb[i*gDisplayContext.screen_width + j]=33554431; // in the frame buffer, a pixel occupies 32-bit.
        }
        p -= rsize; // to preceding row
    }
}
